...
import {
  ...
  TouchableOpacity,
  Modal, 
  Animated, // Importat pentru animații
} from 'react-native';
...
const CITY_OPTIONS = [
    ...
    { label: "Utilizator (Locație Curentă)", value: "User", latitude: 0, longitude: 0 }, 
];
...
export default function HomeScreen({ navigation }) { 
  ...
  const navigateToSearch = () => {
    if (navigation) {
      ...
      console.log("Navigația nu este disponibilă. Nu s-a putut naviga la 'Search'.");
    }
  };
  ...
  const navigateToProfile = () => {
    if (navigation) {
        ...
        console.log("Navigația nu este disponibilă. Nu s-a putut naviga la 'Profile'.");
    }
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: CUSTOM_COLORS.BACKGROUND }]}>
      
      {/* Container comun pentru Search Bar și Coffee Points */}
      <View style={styles.headerContainer}>
        {/* Bara de Căutare (o parte din rând) */}
        <TouchableOpacity onPress={handleSearchPress} style={styles.searchBarWrapper}>
          <TextInput
            style={[styles.searchBar, { backgroundColor: CUSTOM_COLORS.CARD, color: CUSTOM_COLORS.TEXT_PRIMARY }]}
        ...
        {/* Buton Puncte de Cafea (Coffee Points) */}
        <TouchableOpacity 
            style={[styles.coffeePointsButton, { backgroundColor: CUSTOM_COLORS.CARD }]}
            onPress={navigateToProfile}
        >
          {/* ICONIȚA COFFEE BEAN (PNG local) */}
          <Image 
          ...
          {/* Numărul de puncte */}
          <Text style={[styles.coffeePointsText, { color: CUSTOM_COLORS.ACCENT_GOLD }]}>
            {coffeePoints}
          </Text>
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.dropdownContainer}>
        <TouchableOpacity 
            ...
            style={[styles.dropdownButton, { backgroundColor: CUSTOM_COLORS.CARD, borderColor: CUSTOM_COLORS.ACCENT_GOLD }]}
        >
          <Text style={[styles.dropdownText, { color: CUSTOM_COLORS.TEXT_PRIMARY }]}>
            {selectedCity.label}
          </Text>
        </TouchableOpacity>
      </View>
        ...
        >
          <View style={[styles.modalContent, { backgroundColor: CUSTOM_COLORS.CARD }]}>
            {CITY_OPTIONS.map((city) => (
              <TouchableOpacity
                key={city.value}
                style={[styles.modalItem, city.value === selectedCity.value && { backgroundColor: CUSTOM_COLORS.ACCENT_GOLD + '20' }]}
                onPress={() => handleCityChange(city)}
                disabled={city.value === 'User' && !userLocation}
              >
                <Text style={[styles.modalItemText, { color: CUSTOM_COLORS.TEXT_PRIMARY }, city.value === 'User' && !userLocation && { color: CUSTOM_COLORS.TEXT_SECONDARY }]}>
                  {city.label} 
          ...
          >
            ...
            {allPlaces.map((place) => (
              <Marker
                ...
                title={place.name}
                description={`${place.distance.toFixed(2)} km de ${selectedCity.label.split(',')[0]})${place.name.includes(selectedCity.label.split(',')[0]) ? ' (Aici)' : ''}`}
      ...
      <Animated.View 
          style={[styles.feedContainer, { height: feedHeight, backgroundColor: CUSTOM_COLORS.BACKGROUND_FEED_TRANSPARENT }]}
      >
        ...
        >
            <View style={[styles.notchHandle, { backgroundColor: CUSTOM_COLORS.NOTCH_HANDLE }]} />
        </TouchableOpacity>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            ...
            <View style={[styles.cardRow, { backgroundColor: CUSTOM_COLORS.CARD }]}>
                {/* Imaginea la stânga (și mai mică) */}
              <Image 
                  source={{ uri: item.image_url || 'https://placehold.co/100x100/333333/D4AF37?text=LOC' }} 
                  style={styles.photoRow} 
                  onError={(e) => console.log('Image load error:', e.nativeEvent.error)}
              />
              {/* Textul la dreapta (alignat pe verticală) */}
              <View style={styles.cardTextRow}>
                <Text style={[styles.partnerTag, { color: CUSTOM_COLORS.ACCENT_GOLD, borderColor: CUSTOM_COLORS.ACCENT_GOLD }]}>
                  PARTENER
                </Text>
                <Text style={[styles.placeNameRow, { color: CUSTOM_COLORS.TEXT_PRIMARY }]}>
                  {item.name}
                </Text>
                <Text style={[styles.ratingRow, { color: CUSTOM_COLORS.ACCENT_GOLD }]}>
                  ★ {item.rating}
                </Text>
                <Text style={[styles.descriptionRow, { color: CUSTOM_COLORS.TEXT_SECONDARY }]}>
                  {item.short_description}
                </Text>
                <Text style={[styles.distanceRow, { color: CUSTOM_COLORS.TEXT_SECONDARY }]}>
